...
export function installStirInput(canvas: HTMLCanvasElement): StirInput {
  ...
  let decay = 0;
  let lastDownAt = 0;
  let lastDownPos: [number, number] = [0, 0];
  ...
  const down = (event: PointerEvent) => {
    ...
    velocity = [0, 0];
    decay = 2;
    // Double-tap (or double-click) = dye explosion at that spot.
    const now = event.timeStamp;
    const p = point(event);
    if (
      now - lastDownAt < 350 &&
      Math.hypot(p[0] - lastDownPos[0], p[1] - lastDownPos[1]) < 0.06
    ) {
      fireBlast(p[0], p[1], 1);
      lastDownAt = 0; // swallow triple-taps
    } else {
      lastDownAt = now;
      lastDownPos = p;
    }
