...
import { getPlaybackEnergy } from "../shared/energy";
import { getBeatPulse } from "../shared/beat";
import { getBlast } from "../shared/blast";
import { getTheme, hexToRgb01 } from "../shared/theme";
...
function inputUniforms(fluid: Fluid, input?: StirInput) {
  ...
  // Reacting to the raw YouTube waveform directly isn't possible: the audio
  // lives inside YouTube's cross-origin iframe, which the browser doesn't let
  // us tap with an AnalyserNode. Instead the beat engine (src/shared/beat.ts)
  // fuses three sources — a BPM clock, mic listening, and local-file FFT —
  // into one per-frame pulse. The fluid below dances to THAT.
  const energy = getPlaybackEnergy();
  const beat = getBeatPulse();
  const musicIntensity = energy.playing
    ? 0.35 + 0.65 * Math.min(1, energy.volume / 100)
    : 0;
  // Blend: 70% real/BPM beat pulse + 30% gentle synthetic sway so the fluid
  // never looks frozen between kicks.
  const sway = 0.5 + 0.5 * Math.sin(time * 2 * Math.PI * 1.3);
  const beatMix = beat.pulse * 0.7 + sway * 0.3;
  const musicPulse = musicIntensity * (0.35 + 0.65 * beatMix);
  // Kicks briefly widen the emitters' orbit + strength for a visible "thump".
  const kickBoost = beat.kicked ? 0.9 : beat.pulse * 0.35;
  const ampScale = 1 + 0.35 * musicIntensity + 0.25 * beat.pulse * musicIntensity;

  const [a0, b] = idleEmitters(fluid.step, ampScale);
  // Splash blast (double-tap / shake): hijack emitter A to the blast point
  // with a big, fast-decaying boost. No shader change needed.
  const blast = getBlast();
  const a: [number, number] = blast ? [blast.x, blast.y] : a0;
  const blastBoost = blast ? blast.power : 0;
  ...
  const [ar, ag, ab] = hexToRgb01(theme.dyeA);
  const [br, bg, bb] = hexToRgb01(theme.dyeB);
  ...
  return {
    ...
    idle_a: [...a, ramp * idle + musicPulse * 0.6 + kickBoost * 0.35 + blastBoost * 1.4, 0.0035 * (1 + 0.3 * musicIntensity + beat.pulse * 0.5 + blastBoost * 2.4)],
    idle_b: [...b, ramp * idle + musicPulse * 0.55 + kickBoost * 0.3, 0.0032 * (1 + 0.3 * musicIntensity + beat.pulse * 0.5)],
    idle_a_color: blast?.color
      ? [blast.color[0], blast.color[1], blast.color[2], 1]
      : [ar, ag, ab, 1],
    idle_b_color: [br, bg, bb, 1],
  };
}

function idleEmitters(step: number, ampScale: number): [[number, number], [number, number]] {
  const t = step / 60;
  return [
    [
      0.5 + 0.28 * ampScale * Math.sin(0.73 * t),
      0.5 + 0.22 * ampScale * Math.sin(1.09 * t + 0.4),
    ],
    [
      0.5 + 0.26 * ampScale * Math.sin(0.61 * t + Math.PI),
      0.5 + 0.24 * ampScale * Math.sin(0.97 * t + 2.1),
    ],
  ];
}
